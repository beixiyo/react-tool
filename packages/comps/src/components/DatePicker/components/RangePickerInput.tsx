...
export const RangePickerInput = memo<RangePickerInputProps>(({
  ...
  const renderAmpm = (ampm?: string, isActive?: boolean) => (
    use12Hours && ampm && (
      <span
        className={ cn('text-sm uppercase shrink-0', {
    ...
    >
      ...
      <div className="flex flex-1 items-center justify-center min-w-0 h-full">
        <button
          ...
          onClick={ (e) => {
            e.stopPropagation()
            if (!disabled) onInputClick?.('start')
          } }
          onKeyDown={ (event) => handleInputKeyDown(event, 'start') }
        >
        ...
        <button
          ...
          onClick={ (e) => {
            e.stopPropagation()
            if (!disabled) onInputClick?.('end')
          } }
          onKeyDown={ (event) => handleInputKeyDown(event, 'end') }
        >
