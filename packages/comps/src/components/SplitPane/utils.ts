 ...
 * 计算面板的初始宽度
 */
export const calculateInitialWidths = (
  configs: PanelConfig[],
  ...
  // 计算自动宽度面板的宽度
  const autoWidth = autoCount > 0 ? (availableWidth - fixedWidth) / autoCount : 0;
 ...
 * 应用宽度约束
 */
export const applyWidthConstraints = (
  width: number,
  config: PanelConfig,
  collapsed: boolean
): number => {
...
export const loadPersistedState = (
  key: string
): { sizes: number[]; collapsedStates: boolean[]; widthsBeforeCollapse: number[] } | null => {
  try {
