...
const useTestAtoms = createUseAtoms(testAtoms)
    ...
    <div className="p-4 border border-border rounded-lg" style={ { backgroundColor: getColor() } }>
      <div className="space-y-2">
        ...
        <div className="text-sm text-textSecondary">
          已访问的 atom 值: <strong>{accessedValue}</strong>
        </div>
        <div className="text-xs text-textTertiary">
          最后渲染时间: {new Date().toLocaleTimeString()}
        </div>
        <div className="text-xs text-textTertiary">
      ...
      <div className="space-y-6">
        ...
        <div className="p-4 bg-info/10 rounded-lg border border-info/20">
          ...
          <p className="text-sm text-textSecondary leading-relaxed">
            验证当使用 <code className="px-1 py-0.5 bg-surface rounded text-xs">createUseAtoms</code> 时，
            如果组件只访问了部分 atom 的值，当未访问的 atom 状态变更时，组件是否会重新渲染。
          </p>
          <p className="text-sm text-textSecondary leading-relaxed mt-2">
            <strong>预期行为：</strong>由于 <code className="px-1 py-0.5 bg-surface rounded text-xs">createUseAtoms</code> 在组件顶层调用了所有 <code className="px-1 py-0.5 bg-surface rounded text-xs">useAtom</code> hooks，
            理论上所有 atom 都会被订阅，即使不访问其值，变更时也会导致组件重新渲染。
          </p>
          <p className="text-sm text-textSecondary leading-relaxed mt-2">
            <strong>用户猜想：</strong>由于 Proxy 只有访问才会提供属性，可能不会导致不必要的重新渲染。
          </p>
        </div>
...
RenderOptimizationDemo.displayName = 'RenderOptimizationDemo'
