...
export default function CatDogPage() {
  ...
  //   i: number,
  // ): Promise<ImageData> => {
  //   const imgPath = new URL(`./img/${category}/${category}.${i}.webp`, import.meta.url).href
  //   const img = await getImg(imgPath)
  //   if (!img) {
  //     throw new Error(`无法加载图片: ${imgPath}`)
  //   }

  //   const resizedImg = await resizeImg(img, 128, 128)
  //   const image = await getImg(resizedImg)

  //   return {
  //     path: imgPath,
  //     element: img,
  //     tensor: tf.browser.fromPixels(image as HTMLImageElement).div(255), // 归一化
  //     label: category === 'cat'
  //       ? 0
  //       : 1,
  //   }
  ...
  // /** 组件渲染 */
  // return (
  //   <div className="mx-auto h-screen max-w-7xl overflow-auto p-6">
  //     <div className="mb-8">
  //       <Button
  //         type="primary"
  //         size="large"
  ...
  /** 真实类别: */
  //                 { ' ' }
  //                 { testSamples[selectedTestIndex].label === 0
  //                   ? '猫'
  //                   : '狗' }
  //               </div>
  //               { prediction && (
  //                 <div className="mt-2">
  //                   <Tag color={ prediction.label === '猫'
  //                     ? 'blue'
  //                     : 'gold' }>
  /** 预测结果: */
  //                     { ' ' }
